...
#[rstest]
#[tokio::test]
pub async fn test_start_campaign_ok() {
...
#[rstest]
#[tokio::test]
pub async fn test_start_other_user_campaign_fail() {
    let test_prefix = "E2E_START_CAMPAIGN";
    ...
    let response_for_user_2 = start_test_campaign(&user_id_2, &campaign_id, &http_client).await;
    assert_eq!(
        StatusCode::NOT_FOUND.to_string(),
        response_for_user_2.status().to_string()
    );

    let response_for_user_1 = start_test_campaign(&user_id_1, &campaign_id, &http_client).await;
    assert_eq!(
        StatusCode::OK.to_string(),
...
#[rstest]
#[tokio::test]
pub async fn test_start_campaign_after_started_fail() {
    let test_prefix = "E2E_START_CAMPAIGN_";
    ...
    let start_campaign_again_response =
        start_test_campaign(&user_id, &campaign_id, &http_client).await;
    println!(
        "response from test_start_campaign_ok: {:?}",
        start_campaign_again_response
    );
    assert_eq!(
        StatusCode::BAD_REQUEST.to_string(),
        start_campaign_again_response.status().to_string()
    );
}
